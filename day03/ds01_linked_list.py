...
head, prev, curr = None, None, None # 항상 첫 번째 노드, curr바로 앞의 노드, 현재 선택한 노드
...
def printNodes(start):
    curr = start
    if curr is None: return # head 가 비어있을 경우
    else:
...
def insertNode(findData, insertData):
    ...
    if head.getData() == findData:
        ...
        node.setLink(head)  # 현재 head가 가리키는 노드를 새로운 노드의 링크로 연결
        head = node # head는 새 node로 생성
...
def deleteNode(deleteData):
    ...
    while curr.getLink != None:
        ...
        # curr가 끝까지 탐색했음에도 값이 없을 경우 return
...
if __name__ == '__main__': # 시작 모듈일 때
    ...
    for name in dataArray[1:]: # 1번 인덱스 데이터부터 ('정연'~)
        prev = node # 다현이 들어있는 노드를 prev에 할당
        node = Node(name)   #0/정연, 1/쯔위, 2/사나, 3/지효
        prev.setLink(node) # 이전 노드의 링크에 현재 노드 연결
        memory.append(node)

    printNodes(start=head)
    ...
    deleteNode('문별')
    printNodes(head)
    deleteNode('유고') # 없는 데이터 삭제 시 예외 발생
